...
export default function AuditRecordsPublicPage() {
    ...
    useEffect(() => {
        const auth = localStorage.getItem("audit_public_auth");
        if (auth === "true") {
    ...
    const handleLogin = (e: React.FormEvent) => {
        e.preventDefault();
        if (password === "@@r0nT0@st") {
            setIsAuthenticated(true);
            localStorage.setItem("audit_public_auth", "true");
    ...
    const fetchRecords = useCallback(async () => {
        ...
        setLoading(false);
    }, [filterType, currentDate, isAuthenticated]);
            ...
            <div className="min-h-screen flex items-center justify-center bg-gray-50/30 p-4">
                <Card className="w-full max-w-md">
                    <CardHeader>
                        ...
                        <CardDescription>Please enter the access code to view audit records.</CardDescription>
                    </CardHeader>
                    <CardContent>
                        <form onSubmit={handleLogin} className="space-y-4">
                            ...
                            <Button type="submit" className="w-full">Unlock Page</Button>
                        </form>
                    </CardContent>
                </Card>
            </div>
        ...
        <div className="min-h-screen bg-gray-50/30 p-4 md:p-8">
            <div className="max-w-7xl mx-auto space-y-6">
                <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                    <div>
                        <h1 className="text-2xl font-bold tracking-tight text-primary">Audit Records</h1>
                        ...
                        <div className="rounded-md border bg-white overflow-hidden overflow-x-auto">
                            <Table>
                                <TableHeader className="bg-gray-50/50">
                                    <TableRow>
                                        <TableHead className="text-[10px] font-bold uppercase tracking-widest py-3 min-w-[150px]">Submitted At / Elapsed</TableHead>
                                        <TableHead className="text-[10px] font-bold uppercase tracking-widest py-3 text-center">Status</TableHead>
                                        <TableHead className="text-[10px] font-bold uppercase tracking-widest py-3 min-w-[120px]">Interaction Date</TableHead>
                                        <TableHead className="text-[10px] font-bold uppercase tracking-widest py-3 min-w-[150px]">Advocate</TableHead>
                                    ...
                                    {loading ? (
                                        ...
                                        records.map((record) => (
                                            <TableRow key={record.id} className="hover:bg-gray-50/50 transition-colors group text-xs md:text-sm">
                                                ...
                                                <TableCell className="py-3">
                                                    <span className="font-medium">{record.advocate_name || '-'}</span>
                                                </TableCell>
                                                <TableCell className="py-3">
                                                    {record.source_url ? (
                                                        <a 
